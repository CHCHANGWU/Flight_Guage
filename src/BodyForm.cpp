    ...
    , ui(new Ui::BodyForm)
{
    ...
    m_Ceimage = new QPixmap(":/image/ce.png");
    m_Fuimage = new QPixmap(":/image/fu.png");
}
...
void BodyForm::paintEvent(QPaintEvent *event)
{
...
void BodyForm::drawBackground(QPainter &painter)
{
    // 1️⃣ 绘制白色边框：左、上、右
    ...
    painter.drawPixmap(20, 40,m_Ceimage->size().width() * 0.4,m_Ceimage->size().height() * 0.4, *m_Ceimage);
    painter.drawPixmap(60, 380,m_Fuimage->size().width() / 2,m_Fuimage->size().height() / 2, *m_Fuimage);
